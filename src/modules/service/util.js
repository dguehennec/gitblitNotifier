...
var EXPORTED_SYMBOLS = [ "gitblit_notifier_Util" ];

/**
 * Creates a global instance of gitblit_notifier_Util
 * 
 * @constructor
 * @this {Util}
 * 
 */
var gitblit_notifier_Util = {
    /**
     * @private bundle
     */
    _bundle : null
 ...
 */
gitblit_notifier_Util.getBundleString = function(param) {
    try {
 ...
 */
gitblit_notifier_Util.setTimer = function(timer, func, delay) {
    clearTimeout(timer);
 ...
 */
gitblit_notifier_Util.maxStringLength = function(text, length) {
 ...
 */
gitblit_notifier_Util.showNotification = function(title, text, duration, callback, callbackThis) {
    try {
        // Show the notification
        chrome.notifications.create("", { type: "basic", iconUrl : "skin/images/gitblit_notifier.png", title : title, message : text, isClickable : true}, function (notificationId) {
            ...
            gitblit_notifier_Util.setTimer(null, function() {
                chrome.notifications.clear(notificationId, function(wasCleared) {});
            }, duration);
        });
    }
 ...
 * @return {Boolean} true if success
 */
gitblit_notifier_Util.playSound = function() {
 ...
 */
gitblit_notifier_Util.openURL = function(UrlToGoTo) {
 ...
 * @return {String} crc32
 */
gitblit_notifier_Util.crc32 = function(str) {
    var c, n, i, k;
    var crc = 0 ^ (-1);
    var crcTable = [];
    for (n = 0; n < 256; n++) {
 ...
 *            data the data
 */
gitblit_notifier_Util.notifyObservers = function(topic, data) {
 ...
 */
gitblit_notifier_Util.extend = function(base, sub, superPropName) {
    var tmp = function() {
    };
 ...
 */
gitblit_notifier_Util.dump = function(obj, pref) {
    if (!pref && pref !== '') {
        pref = '=> ';
    }
    for (var p in obj) {
        try {
            ...
            if (v) {
                if (typeof(v) === 'object') {
                    console.log("\n");
                    gitblit_notifier_Util.dump(v, pref + p + '.');
                }
 ...
 */
gitblit_notifier_Util.deepFreeze = function(obj) {
    ...
    for ( var propKey in obj) {
        if (obj.hasOwnProperty(propKey)) {
            ...
            if (typeof (prop) === 'object') {
                gitblit_notifier_Util.deepFreeze(prop);
            }
        }
    }
    return obj;
 ...
 */
Object.seal(gitblit_notifier_Util);
