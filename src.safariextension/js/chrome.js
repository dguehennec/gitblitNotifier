    ...
    chrome = {
        ...
        browserAction: {
            setIcon: function(details, callback) {
                ...
                safari.extension.toolbarItems.forEach(function(item) {
                    if (item.identifier === 'GitblitNotifierTab') {
        ...
        cookies: {
            ...
            remove: function(details, callback) {
                ...
                if(tab && details) {
                    ...
                    tab.page.dispatchMessage("GitblitNotifier_removeCookie", details);
            ...
            get: function(details, callback) {
                ...
                if(tab && details) {
                    details.id = (new Date()).getTime();
                    tab.page.dispatchMessage("GitblitNotifier_getCookie", details);
                    chrome._callbacksCookie.push({id: details.id, callback: callback});
                }
                else if(callback) {
                    callback(null);
            ...
            set: function(details, callback) {
                ...
                if(tab && details) {
                    details.id = (new Date()).getTime();
                    tab.page.dispatchMessage("GitblitNotifier_setCookie", details);
                    chrome._callbacksCookie.push({id: details.id, callback: callback});
                }
                else if(callback) {
                    callback(null);
        ...
        tabs: {
                        ...
                        if (item.identifier === 'GitblitNotifierTab') {
                            ...
                            if(item.popover) {
                                ...
                                setTimeout(function() {
                                    ...
                                    var popup = safari.extension.createPopover("GitblitNotifierOption", url, 760, 530);
                                    item.popover = popup;
                                    item.showPopover();
                                }, 100);            
                            } else {
            ...
            reload: function(tabId, callback) {
                chrome.tabs.get(tabId, function(tab) {
                    if(tab) {
                        tab.page.dispatchMessage("GitblitNotifier_reload");
                    }
                    if (callback) {
                        callback(tab);
                    }
            ...
            sendMessage: function(tabId, message) {
                chrome.tabs.get(tabId, function(tab) {
                    if(tab) {
                        tab.page.dispatchMessage("GitblitNotifier_sendMessage", message);
                    }
                });
            }
        },
        ...
        i18n: {
            getMessage: function(key) {
                var value = '';
                try {
                    var locale = chrome.extension.getBackgroundPage().gitblit_notifier_locale.getLocale();
                    if(locale[key]){
