   ...
   if (event.command == "openPopup") {
        safari.extension.toolbarItems.forEach(function(item) {
            if (item.identifier === 'GitblitNotifierTab') {
                if(item.popover) {
                    if(item.popover.identifier !== 'GitblitNotifierPopup') {
                ...
                var popup = safari.extension.createPopover("GitblitNotifierPopup", safari.extension.baseURI + "popup.html", 350, 100);
                item.popover=popup;
                item.showPopover();
            }
        });
...
safari.application.addEventListener("message", function(event) {
    if (event.name == "GitblitNotifier_message") {
    ...
    downloadLocale: function() {
        ...
        xhr.onreadystatechange = function() {
            if (this.readyState == 4) {
                gitblit_notifier_locale.locale =  JSON.parse(this.responseText) || {};
            }
        };
 ...
 */
gitblit_notifier_locale.downloadLocale();
